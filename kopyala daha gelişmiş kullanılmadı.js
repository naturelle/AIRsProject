          ...
          if (islemebasla == 0) {
              ...
              fs.readdirSync(sfolder).forEach((sfile, index, array) => {
                if (sfile.includes("ANS")) {
                  let unitnumber = sfile.slice(10,11)
                  let airno = sfile.slice(0,17)
                  let grupno = sfile.slice(17,sfile.length).replace(/\.rev\d/, "").replace(".rev", "")
                  let aircode = airno.slice(0, 7)
                  airno = airno.replace(aircode, "")
                  aircode = aircode.replace(".", "")
                  airno = airno + "." + aircode
            ...
            //  let airno2 = airno + "." + aircode + grupno
                ...
                for (let folderi = 0; folderi<=20; folderi++) {
                  if (!fs.existsSync(dfolder.replace(/B\d{2}/g, "B0"+(folderi+5)).replace(/\.rev\d/, "").replace(".rev", ""))){
                    klasorsorgu = false
                  }
                  else {
                    klasorsorgu = true
              ...
              if (kopyalama==0) {
                ...
                if (klasorsorgu == false){
                  fs.mkdirSync(dfolder, { recursive: true });
                }
                else {
                  for (let i = 1; i < 100; i++) {
                    dfolder = "\\\\sp000dfs04.ndk.local\\Birimler\\ngd\\ANS\\Dokümanlar\\Kurum Dokümanları\\"+ unitnumber + ". Ünite İnşaat Lisansı Değerlendirme Raporları\\Ek Bilgi Talepleri (EBT)\\NDK\\4 RECEIVED Yanıt Alınanlar\\"+sfile.replace(/\.rev\d/, "").replace(".rev", "")+".rev"+i+"\\";
                    let klasorsorgu2

                    for (let folderi = 0; folderi<=20; folderi++) {
                      if (!fs.existsSync(dfolder.replace(/B\d{2}/g, "B0"+(folderi+5)))){
                        klasorsorgu2 = false
                      }
                      else {
                        klasorsorgu2 = true
                  ...
                  if (err) {
                    ...
                    function(err, rows){
                      ...
                      // db.all(`SELECT eposta FROM kullanicibos WHERE ${gruplike}`, 
                        ...
                        db.all(`SELECT eposta, grup FROM kullanicibos`, 
